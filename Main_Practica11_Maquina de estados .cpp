//Previo 11		Marín Alva Yulen Caleb Alher	
//26/04/26		320089756
#include <iostream>
#include <cmath>
...
int pathState = 0; //cada uno es un paso
...
void Animation() {
	...
	if (dogAnim == 1) {
		...
		case 3: // Girar 90°
			...
			break; //Marín Alva Yulen Caleb Alher

		case 4: // Avanzar en Z negativo (el tramo largo)
			dogPos.z -= 0.001f;
			advanceCounter += 0.001f;
			if (advanceCounter >= 3.5f) { // Aquí llega a Z = -1.4 aprox.
				advanceCounter = 0;
				// Giramos para apuntar directamente al origen (0,0)
				rotationTarget = dogRot + 125.0f;
				pathState = 5;
			}
			break;

		case 5: // Girar para encarar el origen
			dogRot += 0.2f;
			if (dogRot >= rotationTarget) {
				dogRot = rotationTarget;
				pathState = 6;
		...
		case 6: // Diagonal de regreso al origen
			
			dogPos.x -= 0.001f;
			dogPos.z += 0.0007f; // "Atinarle al (0,0)

			// El perro sigue caminando hasta que su posición en X sea casi cero
			if (dogPos.x <= 0.0f) {
				dogPos.x = 0.0f;
				dogPos.z = 0.0f; // Forzando el reset exacto
				advanceCounter = 0;
				rotationTarget = 0.0f; // Mirando al frente de nuevo
				pathState = 7;
			}
			break;

		case 7: // Giro final para quedar como al principio
			dogRot -= 0.2f;
			if (dogRot <= 0.0f) {
				dogRot = 0.0f;
				pathState = 8; // Misión cumplida
			}
			break; //Marín Alva Yulen Caleb Alher
		...
		if (!step) {
			RLegs += 0.1f; FLegs += 0.1f; head += 0.1f; tail += 0.1f; 
