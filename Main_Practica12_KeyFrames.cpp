//Practica 12		Marín Alva Yulen Caleb Alher
//08/05/2026	320089756
#include <iostream>
#include <cmath>
...
float tail = 0.0f;
float fLeftLeg = 0.0f;
float fRightLeg = 25.0f;
float bLeftLeg = 0.0f;
float bRightLeg = 0.0f;
float bodyIncline = 0.0f;

//Pa' hacerse el muertito
float bodyRoll = 0.0f;
float bodyRollInc = 0.0f;
...
void saveFrame(void)
{
	...
	KeyFrame[FrameIndex].bodyRollInc = bodyRollInc;
...
void resetElements(void)
{
	...
	bodyRollInc = KeyFrame[0].bodyRollInc;
...
void interpolation(void)
{
	...
	//Pasos Marín Alva Yulen Caleb Alher
...
void guardarPasos(void) {
	std::ofstream archivo("animacion_perro.txt");
	if (archivo.is_open()) {
		...
		archivo.close();
		std::cout << "SISTEMA: Animacion guardada con exito." << std::endl;
	}
	else {
...
void cargarPasos(void) {
	std::ifstream archivo("animacion_perro.txt");
	if (archivo.is_open()) {
		...
		archivo.close();
		std::cout << "SISTEMA: Se ha detectado y cargado una animacion previa (" << FrameIndex << " cuadros)." << std::endl;
	}
	else {
		std::cout << "SISTEMA: No se encontro animacion cargada. Iniciando buffer limpio." << std::endl;
		FrameIndex = 0;
	}
}
// Deltatime
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//Body Marín Alva Yulen Caleb Alher
		...
		modelTemp = model = glm::rotate(model, glm::radians(-bodyIncline), glm::vec3(1.0f, 0.0f, 0.0f));
		//Pa´hacerse el muertito
		...
		model = glm::rotate(model, glm::radians(fLeftLeg), glm::vec3(-1.0f, 0.0f, 0.0f)); // <-- CORREGIDO
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = glm::rotate(model, glm::radians(fRightLeg), glm::vec3(1.0f, 0.0f, 0.0f)); // <-- CORREGIDO
		//Saludo
		model = glm::rotate(model, glm::radians(fRightLeg * 0.5f), glm::vec3(0.0f, 0.0f, -1.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = glm::rotate(model, glm::radians(bLeftLeg), glm::vec3(1.0f, 0.0f, 0.0f)); // <-- CORREGIDO
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		B_LeftLeg.Draw(lightingShader);

		//Back Right Leg Marín Alva Yulen Caleb Alher
		model = modelTemp;
		model = glm::translate(model, glm::vec3(-0.083f, -0.057f, -0.231f));
		model = glm::rotate(model, glm::radians(bRightLeg), glm::vec3(-1.0f, 0.0f, 0.0f)); // <-- CORREGIDO
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void DoMovement()
{
	...
	if (keys[GLFW_KEY_U])
	{
		bLeftLeg += 0.025f;
		bRightLeg -= 0.025f;
		if (bLeftLeg >= RLegsB) bLeftLeg = RLegsB;
		if (bRightLeg <= FLegsB) bRightLeg = FLegsB;
	}
	if (keys[GLFW_KEY_J])
	{
		bLeftLeg -= 0.025f;
		bRightLeg += 0.025f;
		if (bLeftLeg <= FLegsB) bLeftLeg = FLegsB;
		if (bRightLeg >= RLegsB) bRightLeg = RLegsB;
	}

	if (keys[GLFW_KEY_Z]) {
		fRightLeg += 0.05f;
		if (fRightLeg >= 80.0f) fRightLeg = 80.0f; //saludo
	}
	if (keys[GLFW_KEY_X]) {
		fRightLeg -= 0.05f;
		if (fRightLeg <= -45.0f) fRightLeg = -45.0f;
	}
	...
	if (keys[GLFW_KEY_0]) { // Sentarse
		bodyIncline += 0.025f; // Sumar
		if (bodyIncline >= 60.0f) bodyIncline = 60.0f;
	}
	if (keys[GLFW_KEY_9]) { // Regresar al estado original (Pararse)
		bodyIncline -= 0.025f; // Restar
	...
	if (keys[GLFW_KEY_8]) {
		bodyRoll += 0.066f;
		if (bodyRoll >= 90.0f) bodyRoll = 90.0f; // Se tumba de lado
	}
	if (keys[GLFW_KEY_7]) {
		bodyRoll -= 0.066f;
		if (bodyRoll <= 0.0f) bodyRoll = 0.0f; // Regresa a estar derecho
	}
	/*if (keys[GLFW_KEY_T])
	{
